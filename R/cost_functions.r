#' Calculates the a cost function for model optimization.
#' Different cost functions can be specified
...
#' Calculates the likelihood cost function for
#' model optimization.
#'
#' @param par a vector of parameter values, this is functions specific
#' @param data nested data structure with validation data as returned
...
likelihood = function(par, data, model, ...) {
  ...
  if (any(is.na(out))) {
    ...
    # return the RMSE between the validation data and
    # the output of the model
    singlelikelihoods = dnorm(val, mean = out, sd = 1/(param[4]^2), log = T)
