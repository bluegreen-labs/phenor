...
#' @param k perform k-fold cross validation (set k=0 to perform a leave-one-out cross-validation)
#' @param CV_seed a vector with random seeds for cross validation (to shuffle the input data)
#' @param cv_set  (optional) use custom sets cross validation (requires a list of vectors the indices of the data used for validation (hold-out), the remaining data will be used for calibration)
#' @param ncores  number of cores to use for crossvalidation (requires 'doParallel' and 'foreach'  library)
#' 
# Further Parameters as in model_comparison, i.e
...
model_CV<-function( k=10,CV_seed=1234,cv_set=NULL,ncores=1,
                    random_seeds = c(1,12,40), models = c("LIN","TT","TTs","PTT","PTTs","M1","M1s","AT","SQ","SQb","SM1","SM1b","PA","PAb","PM1","PM1b","UN","UM1","SGSI","AGSI"),
                    data = phenocam_DB, 
                    method = "GenSA", control = list(max.call = 5000,temperature = 10000),
  ...
  
{
  ...
  fold<-function(i){
    ...
    for (j in 1:length(models)){
      ...
      CVfold$modelled[[j]]$validation_predicted_values<-vp
    }
    return(CVfold)
  }
  
  # get years out of the data (not needed)
  #years<-as.vector(do.call("c",lapply(data,function(x)x$year)))
  
  # convert to a flat format for speed
  
  data<-flat_format(data)  # The call to model_copmarison call further down will call this again; but already flat datasets are ignored by the function
  cvindex<-c(1:length(data$transition_date)) 
  
  #Prepare training and validation dataset indices ...
  
  n<-length(cvindex)
  if (is.null(cv_set)){                         # Create the CV sets here
    if (k==0){k<-n}                             # k=0 -> indicates leave-one-out
    vc<-(n-n%%k)/k                              # size of fold
    if (n%%k>0) {print (sprintf("%s-fold CV: Warning: %s random value(s) dropped while creating %s sets of equal size (%s values each)",k,n%%k,k,vc))}
    if (k==n){                                  # LEAVE-ONE-OUT
      rnd_set<-c(1:n)                           # dont randomize
      print ("LEAVE-ONE_OUT CV")
    } else {                                    # K-FOLD 
      set.seed(CV_seed)
      rnd_set<-sample(cvindex)                  # shuffle data
      print (sprintf("%s-FOLD CV",k))
    } 
    cv<-rnd_set[1:(n-n%%k)]                     # cv: shuffeled set of indexes, vc: length of validation data set
    cv_set=list()
    for (i in 1:k){
      vci<-c(1:vc)+vc*(i-1) 
      cv_set[[i]]<-cv[vci]                      # validation sets
    }
  } else {    
  ...
  if (ncores==1){ # Synchronous mode
    #results <-foreach(i=1:k) %do% {fold(i)} 
    results<-list() 
    for (i in 1:k) {results[[i]]=fold(i)}
    
  } else { # Parallel mode
    print (sprintf("Using %s cores to calculate the cross validation (%s sets). This may still take a while...",ncores,k))
    cl<-makeCluster(ncores)
    registerDoParallel() # Use n cores for parallel CV
  ...
  for (m in 1:length(models)){
    ...
    cv_stat[[m]]<-list(training_errors=tmp_trn,validation_errors=tmp_val,mean_training_error=mrmse_trn,mean_validation_error=mrmse_val)
  }
